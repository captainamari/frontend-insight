...
4. v1.6 为保护 M0–M5 既有实现，将“功能采用”保留为默认首页；这与产品流程图要求的“先查看全部已接入项目，再进入单个项目概览”不一致。
...
- 管理端默认入口必须是项目入口页；用户先看到其有权访问的全部已接入项目，再从某个项目进入项目概览；功能采用是项目内专项分析页，不是平台默认入口；
...
1. **平台作用域**：项目入口页展示当前用户有权访问的全部已接入项目。该页可以按项目名称、状态和负责人筛选，但不要求先选择某个项目，也不应用单项目的环境、发布和实体筛选。
2. **项目作用域**：用户点击项目后进入该项目概览，当前 `projectId` 由路由确定。项目概览及其全部专项分析页共享部署环境、时间范围、项目时区、发布版本；可选模块、页面、功能/任务及粗粒度浏览器/操作系统。

项目内下钻必须保留项目、环境、时间和发布上下文，除非用户显式重置。切换项目时必须重新校验授权，并清理不属于新项目的模块、页面、功能/任务筛选。项目入口页的搜索、排序和分页状态应在返回时保留。
...
附件流程图固化的首要关系不是“选择一个分析能力”，而是“先确定项目，再在项目内分析”：

1. 平台默认进入项目入口页，先回答已接入多少项目、分别是什么项目、当前是否有数据以及能否进入；
2. 点击某个项目后进入该项目概览，先看到项目属性、资产规模、项目运营指数状态和各专项能力摘要；
3. 用户再从项目概览进入功能采用、页面与任务、项目运营指数、可观测性或配置页面；
4. 项目内实体继续按项目 → 模块 → 页面 → 功能/任务下钻，并保留分析上下文。

“配置 → 采集 → 计算 → 展示”描述数据生产闭环，不决定管理端首页顺序。默认入口应由用户的导航任务决定，不能因为“功能采用”是核心分析能力，就让用户在尚未选择项目时直接进入功能采用。

```mermaid
flowchart TD
    R["登录 / 授权范围"] --> P["项目入口页（平台默认入口）"]
    P --> O["项目概览"]
    O --> A["功能采用"]
    O --> U["使用、页面与任务"]
    O --> I["项目运营指数"]
...
| 页面 | 回答的问题 | 主指标/内容 | 主要去向 |
| --- | --- | --- | --- |
| 项目入口页（平台默认） | 当前接入了哪些项目，我能进入哪个项目 | 已授权项目总数、项目名称/状态/负责人、模块/页面/功能/任务资产数、最近数据时间与数据状态；运营指数满足门槛时显示分数，否则显示不可用原因 | 点击项目 → 项目概览；admin 可进入新增/接入 |
| 项目概览（项目默认） | 这个项目是什么、整体运行和使用情况怎样、下一步应看哪里 | 项目属性、资产规模、环境/发布、数据状态、运营指数摘要、功能采用/使用效率/错误性能摘要及趋势 | 功能采用、使用与页面、任务、指数、可观测性、配置 |
| 功能采用 | 功能是否被看到、开始并成功使用 | 曝光、开始、成功/失败、曝光后使用、持续使用 | 功能/任务详情 |
| 使用、页面与任务 | 项目、模块和页面是否持续使用，任务是否顺畅 | 活跃账号/浏览器/会话、WAU、时长、覆盖、深度、关键任务、未归类 route | 模块 → 页面 → 功能/任务 |
...
项目入口页可以展示项目运营指数，但它只是项目级摘要：必须复用项目运营指数 read model，并同时展示数据状态、样本/门槛或不可用原因。不得在入口页另算“项目分数”，也不得把无数据、样本不足和真实 0 分合并。

### 9.2 路由与导航规则

- `/` 登录成功后必须进入或重定向到 `/projects`；`/projects` 是平台默认入口，不得重定向到功能采用、最近访问项目或列表中的第一个项目。
- `/projects` 必须一次展示当前用户有权访问的全部已接入项目；项目较多时可以分页或虚拟化，但“项目总数”和当前筛选结果数必须区分。
- 点击项目卡片或表格行进入 `/projects/:projectId/overview`。该页面是单个项目的默认详情页，不得直接跳到 `/projects/:projectId/adoption`。
- 项目内专项页面使用稳定子路由，例如 `/adoption`、`/usage`、`/pages`、`/tasks`、`/operational-index`、`/observability` 和 `/settings`；最终路径可由实现 ADR 固化，但层级和默认关系不得改变。
- 直接访问项目内深链时必须校验项目权限；无权限返回 `forbidden`，项目不存在返回 `not_found`，不得退回另一个项目造成上下文错配。
- 从项目内页面返回项目入口页时，保留入口页的搜索、排序、分页和状态筛选；浏览器刷新和可分享 URL 不丢失项目内环境、时间、发布及实体筛选。
- 即使用户只有一个授权项目，也默认显示项目入口页；若未来需要“自动进入唯一项目”，必须作为新的显式产品决策评审，不能由前端自行优化。

### 9.3 项目入口与项目概览的最小展示契约

项目入口页每个项目至少返回并展示：`projectId`、项目名称、项目状态、当前用户权限、负责人/所属信息（已配置时）、模块/页面/功能/任务数量、最近有效事件时间、数据状态，以及运营指数值或不可用原因。跨项目列表不得返回项目内高基数指标或个人维度。

项目概览至少展示：项目基础属性、资产数量与配置覆盖、当前环境/时间/发布上下文、数据新鲜度、项目运营指数摘要、功能采用摘要、使用与任务摘要、错误/性能摘要。每个摘要卡片必须可以进入对应专项页，并复用相同 read model、定义版本和筛选上下文。

### 9.4 通用交互规则

- 项目作用域内的筛选器在所有分析页面位置和语义一致，并同步到 URL，支持刷新和分享。
...
- AI 入口未来可在任意项目内页面出现，但只读取用户勾选的卡片/趋势/错误组快照；项目入口页不默认拼接多个项目数据，页面上下文也不是全库权限。
...
- 提供按当前用户权限返回项目集合的项目入口 read model；支持稳定排序、筛选和分页，并返回总项目数与筛选结果数；
- 提供单项目概览 read model，聚合项目属性、资产数量、数据状态、运营指数及各专项摘要；摘要与专项页复用同一指标定义和查询口径，避免 N+1 查询和前端拼装公式；
- 返回规范字段、metric definition 摘要、numerator/denominator/sample/coverage 和 data status；
- 项目内接口接受统一 project/environment/release/time/entity 筛选；项目集合接口不接受会造成跨项目口径混淆的单项目实体筛选；
...
项目入口和项目概览的具体 endpoint 名称由实施 ADR 固化，但必须分别拥有契约和 RBAC 测试。项目入口返回的运营指数摘要必须包含值、状态、版本和不可用原因；不得由 Web 端跨项目调用多个专项接口后自行计算。
...
| 优先级 | 内容 | 说明 |
| --- | --- | --- |
| P0 | Pre-1.0 数据重置、唯一命名、项目入口页、项目概览、页面关系、MetricCatalog、现有指标 P90/分母/状态统一 | 不新增敏感采集；先纠正默认入口和项目内下钻，再消除同义词与错误解释 |
...
- 登录后访问 `/` 必须到达展示全部已授权接入项目的项目入口页；功能采用不得是平台默认入口。
- 项目入口页正确展示总项目数、筛选结果数和每个项目的最小摘要；无项目时 viewer 看到明确空态，admin 看到新增/接入路径。
- 点击任一项目必须进入该项目概览，而不是功能采用或其他专项页；项目概览可进入功能采用、使用/页面/任务、指数、可观测性和配置。
- 项目入口 → 项目概览 → 模块 → 页面 → 功能/任务下钻可达；返回项目入口时保留搜索、排序、分页和状态筛选。
- 项目内环境、时间、发布和实体筛选跨专项页保持一致；切换项目时移除不属于新项目的实体筛选。
- 项目入口和项目概览的分数/摘要复用统一 read model；无数据、样本不足、不可用和真实 0 分显示不同状态。
- 深链权限、`forbidden`、`not_found`、单项目授权和零项目授权均有 Chromium/WebKit E2E。
...
5. 平台默认入口确定为项目入口页；点击项目后先进入项目概览；功能采用仅作为项目内专项分析页。
