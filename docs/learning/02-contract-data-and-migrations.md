...
- `packages/event-contract/schema/event-batch.schema.json` 是网络传输格式的唯一事实来源；
...
| -------------- | ----------------------------- | ----------------------------------------------------- | --------------------------------- |
| 浏览器传输批次 | `FrontendInsightEventBatchV1` | `projectKey`、`accountRef?`、SDK 信息                 | 没有内部 `projectId`、`accountId` |
...
7. 更新 `docs/contracts/event-contract-v1.md`。

兼容判断：新增不改变旧语义的可选字段可以留在 v1；删除、改名、改变类型/必填性或含义必须升 `schemaVersion`。引入 v2 后，设计要求至少保留 v1/v2 兼容窗口，不能直接让旧 SDK 全部被拒绝。
...
项目、功能和用户都有 `status`/`disabled_at`。软停用保留历史事件与审计关联，避免一次 UI 操作物理删除 ClickHouse 大量数据。真正清理应进入独立审批/运维流程。
