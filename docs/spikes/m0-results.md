...
| Compose 结构、镜像固定、端口隔离 | YAML 结构测试 + `docker compose config` | 结构测试通过；Compose CLI 由 CI 执行 |
| MySQL migration、连接池、读写 | `verify.mjs` | 等待远程 Compose CI |
| ClickHouse Node 批写、DateTime64(3) | `verify.mjs` | 等待远程 Compose CI |
| Kafka topic、produce/consume | `verify.mjs` | 等待远程 Compose CI |
| 浏览器事件进入 ClickHouse | API + Kafka + consumer + ClickHouse 轮询 | 等待远程 Compose CI |
| 原始账号引用不进入 Kafka/ClickHouse | HMAC 结果断言 + 禁止字段测试 | 单元测试通过；全链路等待 CI |
| Apple Silicon 原生镜像与资源 | 目标 M1 Mac 的 `doctor/status` 输出 | 等待产品负责人本地验收 |

提交前本工作区执行 `npm test`：10 项测试通过，0 项失败；`scripts/dev` 通过 Bash 语法检查。当前执行环境没有 Docker，因此容器项必须以远程 CI 和目标 Mac 的结果为准。
