...
CREATE TABLE IF NOT EXISTS modules (
  ...
  KEY idx_modules_lifecycle (project_id, archived_at, status),
...
CREATE TABLE IF NOT EXISTS module_revisions (
  ...
  created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  UNIQUE KEY uq_module_revisions_version (module_id, revision),
  UNIQUE KEY uq_module_revisions_window_start (module_id, effective_from),
  KEY idx_module_revisions_window (module_id, effective_from, effective_to),
  CONSTRAINT chk_module_revisions_version CHECK (revision > 0),
  CONSTRAINT chk_module_revisions_status CHECK (status IN ('active', 'disabled')),
  CONSTRAINT chk_module_revisions_window CHECK (effective_to IS NULL OR effective_to > effective_from),
  CONSTRAINT fk_module_revisions_module FOREIGN KEY (module_id) REFERENCES modules (id) ON DELETE RESTRICT,
  CONSTRAINT fk_module_revisions_user FOREIGN KEY (created_by_user_id) REFERENCES users (id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS page_definitions (
  ...
  KEY idx_page_definitions_lifecycle (project_id, archived_at, status),
...
CREATE TABLE IF NOT EXISTS page_definition_revisions (
  id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin PRIMARY KEY,
  page_definition_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  revision INT UNSIGNED NOT NULL,
  module_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  name VARCHAR(120) NOT NULL,
  ...
  created_by_user_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  UNIQUE KEY uq_page_revisions_version (page_definition_id, revision),
  UNIQUE KEY uq_page_revisions_window_start (page_definition_id, effective_from),
  KEY idx_page_revisions_module_window (module_id, effective_from, effective_to),
  CONSTRAINT chk_page_revisions_version CHECK (revision > 0),
  CONSTRAINT chk_page_revisions_template CHECK (template_key IN ('monitoring_dashboard', 'analysis_view', 'task_operation')),
  CONSTRAINT chk_page_revisions_frequency CHECK (expected_frequency IN ('daily', 'weekly', 'monthly', 'ad_hoc')),
  CONSTRAINT chk_page_revisions_status CHECK (status IN ('active', 'disabled')),
  CONSTRAINT chk_page_revisions_weight CHECK (criticality_weight > 0 AND criticality_weight <= 100),
  CONSTRAINT chk_page_revisions_window CHECK (effective_to IS NULL OR effective_to > effective_from),
  CONSTRAINT fk_page_revisions_page FOREIGN KEY (page_definition_id) REFERENCES page_definitions (id) ON DELETE RESTRICT,
  CONSTRAINT fk_page_revisions_module FOREIGN KEY (module_id) REFERENCES modules (id) ON DELETE RESTRICT,
  CONSTRAINT fk_page_revisions_user FOREIGN KEY (created_by_user_id) REFERENCES users (id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS workflow_definitions (
  ...
  KEY idx_workflow_definitions_module (module_id, archived_at, status),
...
CREATE TABLE IF NOT EXISTS workflow_definition_versions (
  ...
  CONSTRAINT chk_workflow_version_window CHECK (effective_to IS NULL OR (activated_at IS NOT NULL AND effective_to > activated_at)),
  ...
  CONSTRAINT fk_workflow_versions_module FOREIGN KEY (module_id) REFERENCES modules (id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS metric_library_versions (
  id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin PRIMARY KEY,
  project_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  library_type VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  version INT UNSIGNED NOT NULL,
  status VARCHAR(16) NOT NULL DEFAULT 'draft',
  manifest_version VARCHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  source_version_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NULL,
  created_by_user_id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  activated_at TIMESTAMP(3) NULL,
  superseded_at TIMESTAMP(3) NULL,
  abandoned_at TIMESTAMP(3) NULL,
  created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  ...
  CONSTRAINT fk_metric_library_project FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE RESTRICT,
  CONSTRAINT fk_metric_library_source FOREIGN KEY (source_version_id) REFERENCES metric_library_versions (id) ON DELETE RESTRICT,
  CONSTRAINT fk_metric_library_creator FOREIGN KEY (created_by_user_id) REFERENCES users (id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS metric_definitions (
  ...
  formula_description VARCHAR(2000) NOT NULL,
  numerator_definition VARCHAR(1000) NULL,
  ...
  reporting_timing VARCHAR(1000) NOT NULL,
  entity_scopes JSON NOT NULL,
  time_granularities JSON NOT NULL,
  minimum_sample INT UNSIGNED NOT NULL,
  missing_policy VARCHAR(1000) NOT NULL,
  owner VARCHAR(120) NOT NULL,
  definition_version VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  ...
  milestone VARCHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  enabled BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  UNIQUE KEY uq_metric_definitions_key (library_version_id, metric_key),
  CONSTRAINT chk_metric_definition_origin CHECK (origin IN ('system', 'business')),
